...
+ (void) initialize
{
    ...
    if (tisSource) {
        ...
        if (layoutData) {
            ...
            for (int keyCode = 0; keyCode < 255; keyCode++) {
                ...
                if (keyCodeLookupTable[keyCodeObj])
                    continue;
                ...
                if (err == noErr && realLength > 0) {
                    NSString *keyName = [[NSString stringWithCharacters:chars
                                                                 length:1] uppercaseString];
                    keyCodeLookupTable[keyCodeObj] = keyName;
                }
            }
        }
    }
    ...
    reverseKeyCodeLookupTable = [NSMutableDictionary new];
    [keyCodeLookupTable enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        if (keyName.length > 0) {
            if ((existingCode = reverseKeyCodeLookupTable[keyName])) {
            ...
            reverseKeyCodeLookupTable[keyName] = keyCodeObj;
        }
    }];
}
...
{
    NSString *keyName = [AKKeyCaptureView descriptionForKeyCode:theEvent.keyCode];
...
+ (NSString *) descriptionForKeyCode:(NSInteger) keyCode
{
    if (keyCode != AKKeyNone) {
        NSString *string = keyCodeLookupTable[@(keyCode)];
        if (string)
            return string;
    }
...
+ (NSInteger) keyCodeForDescription:(NSString *) description
{
    if (description && description.length > 0) {
        NSNumber *keyCode = reverseKeyCodeLookupTable[description];
        if (keyCode)
...
- (void) drawRect:(NSRect) dirtyRect
{
    ...
    if (self.canUnmap) {
        ...
        mpstyle.alignment = self.alignment;
